...
export default function CreatePost({ onPostCreated }) {
  ...
  const handleAISuggestTags = () => {
    ...
    suggested.forEach(t => { if (!newTags.includes(t)) newTags.push(t); });
    setHashtags(newTags);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API}/api/posts`, fd);
  ...
  return (
    <>
      {!isOpen && (
        <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 20, padding: '20px 24px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
          <img src={avatar} style={{ width: 46, height: 46, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--primary)' }} alt="me" />
          <button 
            onClick={() => setIsOpen(true)}
            style={{ flex: 1, textAlign: 'left', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)', borderRadius: 30, padding: '14px 24px', color: '#94a3b8', fontSize: 15, cursor: 'pointer', transition: 'all 0.25s ease' }}
      ...
      {isOpen && (
        <div className="modal-overlay-blur" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(5, 7, 10, 0.75)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflowY: 'auto' }}>
          <div className="premium-modal-card" style={{ background: 'var(--card-bg)', border: '1px solid rgba(99, 102, 241, 0.25)', borderRadius: 24, padding: '32px', color: 'var(--text)', boxShadow: '0 25px 70px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.05)', maxWidth: '880px', width: '100%', margin: 'auto', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, borderBottom: '1px solid var(--border)', paddingBottom: 18 }}>
              ...
              <button onClick={() => setIsOpen(false)} className="close-studio-btn" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', color: '#94a3b8', width: 34, height: 34, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, transition: 'all 0.2s' }}>✕</button>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Type an immersive article title or use generator engine..." className="studio-input-glow" style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px 165px 14px 16px', fontSize: 15, color: '#f8fafc', outline: 'none', transition: 'all 0.25s' }} />
                <button type="button" onClick={handleGenerateTitles} disabled={isGeneratingTitle} style={{ position: 'absolute', right: 10, background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', border: 'none', color: '#fff', fontSize: 11, fontWeight: 700, padding: '8px 14px', borderRadius: 10, cursor: 'pointer', boxShadow: '0 4px 12px rgba(99,102,241,0.35)' }}>
              ...
              {showSuggestions && (
                <div className="ai-dropdown-anim" style={{ background: '#090d16', border: '1px solid #4f46e5', borderRadius: 14, marginTop: 12, overflow: 'hidden', boxShadow: '0 15px 35px rgba(0,0,0,0.6)' }}>
                  ...
                  {aiTitlesList.map((t, idx) => (
                    <div key={idx} onClick={() => { setTitle(t); setShowSuggestions(false); }} className="ai-suggest-item" style={{ padding: '12px 16px', fontSize: 13, color: '#cbd5e1', cursor: 'pointer', borderBottom: '1px solid #111827', transition: 'all 0.2s' }}>{t}</div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24, marginBottom: 24 }}>
              <div>
                ...
                <select value={category} onChange={(e) => setCategory(e.target.value)} className="studio-input-glow" style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)', borderRadius: 14, padding: 14, fontSize: 14, color: '#e2e8f0', outline: 'none', cursor: 'pointer' }}>
              ...
              <div>
                ...
                <div onClick={() => htagInputRef.current.focus()} className="studio-input-glow" style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)', borderRadius: 14, padding: '8px 12px', minHeight: '48px', display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', cursor: 'text' }}>
                  {hashtags.map((tag) => (
                  ...
                  <input ref={htagInputRef} type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={handleAddTag} placeholder={hashtags.length === 0 ? "Type metadata tag & click Enter..." : ""} style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: 13, color: '#fff', minWidth: 100, flex: 1 }} />
                </div>
                <button type="button" onClick={handleAISuggestTags} style={{ marginTop: 6, fontSize: 11, color: '#818cf8', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontWeight: 600 }}>✨ AI Suggest Random Macro-Tags</button>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {['💡 Informative', '🔥 Inspiring', '😄 Casual', '🎓 Educational', '💼 Professional', '🌀 Storytelling'].map((t) => (
                  <button type="button" key={t} onClick={() => setMood(t)} style={{ fontSize: 12, padding: '8px 16px', borderRadius: 30, border: '1px solid var(--border)', cursor: 'pointer', transition: 'all 0.2s ease', fontWeight: 600, background: mood === t ? 'rgba(99, 102, 241, 0.25)' : 'rgba(255,255,255,0.02)', color: mood === t ? '#fff' : '#94a3b8', borderColor: mood === t ? '#6366f1' : 'var(--border)', boxShadow: mood === t ? '0 4px 14px rgba(99,102,241,0.2)' : 'none' }}>{t}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ position: 'relative' }}>
                <textarea value={shortDesc} onChange={(e) => setShortDesc(e.target.value)} rows="2" placeholder="Provide a 2-line systemic overview wrapper..." className="studio-input-glow" style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border)', borderRadius: 14, padding: '14px 14px 44px 14px', fontSize: 14, color: '#f1f5f9', outline: 'none', resize: 'none', lineHeight: 1.5 }} />
                <button type="button" onClick={handleAutoWriteDesc} style={{ position: 'absolute', right: 12, bottom: 12, background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', border: 'none', color: '#fff', fontSize: 11, fontWeight: 600, padding: '6px 12px', borderRadius: 8, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>✦ Auto Draft Random Summary</button>
              </div>
            </div>

            <div style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: 0 }}>Primary Editorial Canvas Content <span style={{ color: '#ef4444' }}>*</span></label>
                <span style={{ fontSize: 11, color: '#6366f1', fontWeight: 600 }}>✨ One-Click AI Injector Modes:</span>
              </div>
              <div style={{ background: '#090d16', border: '1px solid var(--border)', borderBottom: 'none', borderRadius: '14px 14px 0 0', padding: '10px 14px', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <textarea value={text} onChange={e => setText(e.target.value)} rows={9} placeholder={`Formulate strategic logs... What insights are we deployment ready with today, ${user?.username}?`} className="studio-input-glow" style={{ width: '100%', resize: 'vertical', border: '1px solid var(--border)', outline: 'none', background: 'rgba(0,0,0,0.3)', fontSize: 15, color: '#f8fafc', lineHeight: '1.7', padding: 16, borderRadius: '0 0 14px 14px', fontFamily: 'inherit' }} />
              ...
              <div style={{ marginTop: 14, background: 'linear-gradient(135deg, rgba(79,70,229,0.1), rgba(124,58,237,0.05))', border: '1px solid #4338ca', borderRadius: 14, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                ...
                <button type="button" onClick={handlePolishContent} className="polish-copilot-btn" style={{ background: '#312e81', border: '1px solid #4f46e5', color: '#fff', fontSize: 12, fontWeight: 700, padding: '8px 14px', borderRadius: 10, cursor: 'pointer', whitespace: 'nowrap', transition: 'all 0.2s' }}>Optimize Draft ✦</button>
              </div>
            </div>
            ...
            {preview && (
              <div style={{ position: 'relative', marginTop: 14, marginBottom: 24 }}>
                ...
                <button type="button" onClick={() => { setImage(null); setPreview(null); }} style={{ position: 'absolute', top: 12, right: 12, width: 30, height: 30, borderRadius: '50%', border: 'none', background: 'rgba(0,0,0,0.85)', color: '#fff', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>✕</button>
              </div>
            )}
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 18, marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button type="button" onClick={() => setIsScheduling(!isScheduling)} style={{ width: 44, height: 24, borderRadius: 20, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '2px 4px', transition: 'background-color 0.25s ease', background: isScheduling ? 'var(--primary)' : '#1e293b' }}>
                  <div style={{ width: 16, height: 16, background: '#fff', borderRadius: '50%', boxShadow: '0 2px 5px rgba(0,0,0,0.5)', transition: 'transform 0.25s cubic-bezier(0.4, 0, 0.2, 1)', transform: isScheduling ? 'translateX(20px)' : 'translateX(0px)' }} />
                </button>
              </div>
              {isScheduling && (
                <input type="datetime-local" value={scheduleTime} onChange={(e) => setScheduleTime(e.target.value)} className="studio-input-glow animate-slide" style={{ marginTop: 14, background: '#090d16', border: '1px solid #334155', borderRadius: 10, padding: '8px 14px', fontSize: 13, color: '#fff', outline: 'none' }} />
              )}
            </div>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border)', paddingTop: 20 }}>
              <label className="upload-media-btn" style={{ cursor: 'pointer', fontSize: 13, fontWeight: 700, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.01)', padding: '10px 16px', borderRadius: 12, border: '1px solid var(--border)', transition: 'all 0.2s' }}>
                📷 Upload Graphic Cover
                <input type="file" accept="image/*" onChange={handleImage} style={{ display: 'none' }} />
              </label>
              <div style={{ display: 'flex', gap: 12 }}>
                <button type="button" onClick={() => { Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Draft locked securely inside local cache!', showConfirmButton: false, timer: 2000 }); setIsOpen(false); }} className="save-draft-btn" style={{ background: 'transparent', border: '1px solid var(--border)', color: '#94a3b8', fontSize: 13, fontWeight: 600, padding: '12px 20px', borderRadius: 14, cursor: 'pointer', transition: 'all 0.2s' }}>💾 Save Draft</button>
                <button onClick={handleSubmit} disabled={loading} className="publish-live-btn" style={{ padding: '12px 26px', borderRadius: 14, border: 'none', cursor: 'pointer', background: 'var(--primary)', color: '#fff', fontWeight: 800, fontSize: 14, opacity: loading ? 0.6 : 1, boxShadow: '0 6px 20px rgba(99,102,241,0.4)', transition: 'all 0.2s' }}>
      ...
      <style>{`
        .premium-modal-card { animation: cinematicPop 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
        @keyframes cinematicPop { from { opacity: 0; transform: scale(0.94) translateY(10px); } to { opacity: 1; transform: scale(1) translateY(0); } }
        .studio-input-glow:focus { border-color: #6366f1 !important; box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.15) !important; }
        .ai-dropdown-anim { animation: slideDownIn 0.2s ease-out forwards; }
        @keyframes slideDownIn { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: translateY(0); } }
        .ai-suggest-item:hover { background: #111422 !important; color: #a5b4fc !important; padding-left: 20px !important; }
        .ai-feature-badge { transition: all 0.2s ease; }
        .ai-feature-badge:hover { transform: translateY(-2px); filter: brightness(1.2); }
        .close-studio-btn:hover { background: #ef4444 !important; color: #fff !important; border-color: #ef4444 !important; }
        .upload-media-btn:hover { border-color: #6366f1 !important; background: rgba(99, 102, 241, 0.05) !important; }
        .save-draft-btn:hover { border-color: #ef4444 !important; color: #ef4444 !important; }
        .publish-live-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(99,102,241,0.55) !important; }
        .animate-slide { animation: slideDownIn 0.25s cubic-bezier(0.4, 0, 0.2, 1); }
      `}</style>
    </>
  );
}
